 ...
 *
 * File:Lunar.java Project: LvPhotoService_1119
 * 
 * Creator:4399-lvtu-8 
 * Date:Apr 26, 2013 10:56:42 AM
 * 
 */
 ...
 * @author joyphper.net
 */
public class LunarUtility {
    public static void main(String[] args) {
        Calendar clr = Calendar.getInstance();
        clr.set(2013, 5, 12);
        LunarUtility lunar = new LunarUtility(clr.getTimeInMillis());
        ...
        System.out.println(LunarUtility.Animals[lunar.getDeqiD()] + "日冲" + LunarUtility.Animals[(lunar.getDeqiD() + 6) % 12] + "生年");
    ...
    private final static int[]       lunarInfo     = { 0x4bd8, 0x4ae0, 0xa570, 0x54d5, 0xd260, 0xd950, 0x5554, 0x56af, 0x9ad0, 0x55d2, 0x4ae0, 0xa5b6, 0xa4d0, 0xd250, 0xd295, 0xb54f, 0xd6a0, 0xada2, 0x95b0, 0x4977, 0x497f, 0xa4b0, 0xb4b5, 0x6a50, 0x6d40,
            0xab54, 0x2b6f, 0x9570, 0x52f2, 0x4970, 0x6566, 0xd4a0, 0xea50, 0x6a95, 0x5adf, 0x2b60, 0x86e3, 0x92ef, 0xc8d7, 0xc95f, 0xd4a0, 0xd8a6, 0xb55f, 0x56a0, 0xa5b4, 0x25df, 0x92d0, 0xd2b2, 0xa950, 0xb557, 0x6ca0, 0xb550, 0x5355, 0x4daf, 0xa5b0,
            0x4573, 0x52bf, 0xa9a8, 0xe950, 0x6aa0, 0xaea6, 0xab50, 0x4b60, 0xaae4, 0xa570, 0x5260, 0xf263, 0xd950, 0x5b57, 0x56a0, 0x96d0, 0x4dd5, 0x4ad0, 0xa4d0, 0xd4d4, 0xd250, 0xd558, 0xb540, 0xb6a0, 0x95a6, 0x95bf, 0x49b0, 0xa974, 0xa4b0, 0xb27a,
            0x6a50, 0x6d40, 0xaf46, 0xab60, 0x9570, 0x4af5, 0x4970, 0x64b0, 0x74a3, 0xea50, 0x6b58, 0x5ac0, 0xab60, 0x96d5, 0x92e0, 0xc960, 0xd954, 0xd4a0, 0xda50, 0x7552, 0x56a0, 0xabb7, 0x25d0, 0x92d0, 0xcab5, 0xa950, 0xb4a0, 0xbaa4, 0xad50, 0x55d9,
            0x4ba0, 0xa5b0, 0x5176, 0x52bf, 0xa930, 0x7954, 0x6aa0, 0xad50, 0x5b52, 0x4b60, 0xa6e6, 0xa4e0, 0xd260, 0xea65, 0xd530, 0x5aa0, 0x76a3, 0x96d0, 0x4afb, 0x4ad0, 0xa4d0, 0xd0b6, 0xd25f, 0xd520, 0xdd45, 0xb5a0, 0x56d0, 0x55b2, 0x49b0, 0xa577,
            0xa4b0, 0xaa50, 0xb255, 0x6d2f, 0xada0, 0x4b63, 0x937f, 0x49f8, 0x4970, 0x64b0, 0x68a6, 0xea5f, 0x6b20, 0xa6c4, 0xaaef, 0x92e0, 0xd2e3, 0xc960, 0xd557, 0xd4a0, 0xda50, 0x5d55, 0x56a0, 0xa6d0, 0x55d4, 0x52d0, 0xa9b8, 0xa950, 0xb4a0, 0xb6a6,
            0xad50, 0x55a0, 0xaba4, 0xa5b0, 0x52b0, 0xb273, 0x6930, 0x7337, 0x6aa0, 0xad50, 0x4b55, 0x4b6f, 0xa570, 0x54e4, 0xd260, 0xe968, 0xd520, 0xdaa0, 0x6aa6, 0x56df, 0x4ae0, 0xa9d4, 0xa4d0, 0xd150, 0xf252, 0xd520 };
    ...
    private final static String[]    solarTerm     = { "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑", "大暑", "立秋", "处暑", "白露", "秋分", "寒露", "霜降", "立冬", "小雪", "大雪", "冬至" };
    private final static String[]    solarTerm_    = { "立春", "春分", "清明", "立夏", "夏至", "立秋", "秋分", "立冬", "冬至" };
     ...
     * 国历节日 *表示放假日
     */
    private final static String[]    sFtv          = { "0101*元旦", "0214 情人节", "0308 妇女节", "0312 植树节", "0401 愚人节", "0501*劳动节", "0601 儿童节", "0808 父亲节", "1001*国庆节", "1111 光棍节", "1224 平安夜", "1225 圣诞节" };
    /**
     * 农历节日 *表示放假日
     */
    private final static String[]    lFtv          = { "0101*春节", "0115 元宵节", "0505*端午节", "0707 七夕情人节", "0815*中秋节", "1208 腊八节 x", "1224 小年", "0100*除夕" };
    /**
     * 某月的第几个星期几
     */
    private static String[]          wFtv          = { "0531 母亲节" };
    ...
    private synchronized void findFestival() {
        ...
        Matcher m;
        for (int i = 0; i < LunarUtility.sFtv.length; i++) {
            m = LunarUtility.sFreg.matcher(LunarUtility.sFtv[i]);
            if (m.find()) {
                if (sM == LunarUtility.toInt(m.group(1)) && sD == LunarUtility.toInt(m.group(2))) {
        ...
        for (int i = 0; i < LunarUtility.lFtv.length; i++) {
            m = LunarUtility.sFreg.matcher(LunarUtility.lFtv[i]);
            if (m.find()) {
                if (lM == LunarUtility.toInt(m.group(1)) && lD == LunarUtility.toInt(m.group(2))) {
        ...
        // 月周节日
        int w, d;
        for (int i = 0; i < LunarUtility.wFtv.length; i++) {
            m = LunarUtility.wFreg.matcher(LunarUtility.wFtv[i]);
            if (m.find()) {
                if (this.getSolarMonth() == LunarUtility.toInt(m.group(1))) {
                    w = LunarUtility.toInt(m.group(2));
                    d = LunarUtility.toInt(m.group(3));
    ...
    private static int getLunarLeapMonth(int lunarYear) {
        ...
        int leapMonth = LunarUtility.lunarInfo[lunarYear - 1900] & 0xf;
    ...
    private static int getLunarLeapDays(int lunarYear) {
        ...
        // 若该年没有闰月,返回0
        return LunarUtility.getLunarLeapMonth(lunarYear) > 0 ? ((LunarUtility.lunarInfo[lunarYear - 1899] & 0xf) == 0xf ? 30 : 29) : 0;
    }
    ...
    private static int getLunarYearDays(int lunarYear) {
        ...
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            daysInLunarYear += ((LunarUtility.lunarInfo[lunarYear - 1900] & i) != 0) ? 1 : 0;
        }
        // 加上闰月天数
        daysInLunarYear += LunarUtility.getLunarLeapDays(lunarYear);
    ...
    private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
        // 数据表中,每个农历年用16bit来表示,
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月
        int daysInLunarMonth = ((LunarUtility.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30 : 29;
    ...
    private static synchronized void makeUTCCalendar() {
        if (LunarUtility.utcCal == null) {
            LunarUtility.utcCal = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
        }
    }
    ...
    private static int getSolarTermDay(int solarYear, int index) {
        ...
        l = l + LunarUtility.UTC(1900, 0, 6, 2, 5, 0);
        return LunarUtility.getUTCDay(new Date(l));
    }
     ...
     * @param TimeInMillis
     */
    public LunarUtility(long TimeInMillis) {
    ...
    private void init(long TimeInMillis) {
        ...
        this.lunarYear = 1900;
        int daysInLunarYear = LunarUtility.getLunarYearDays(this.lunarYear);
        while (this.lunarYear < 2100 && offset >= daysInLunarYear) {
            offset -= daysInLunarYear;
            daysInLunarYear = LunarUtility.getLunarYearDays(++this.lunarYear);
        }
        // 农历年数字
        ...
        int lunarMonth = 1;
        // 所在农历年闰哪个月,若没有返回0
        int leapMonth = LunarUtility.getLunarLeapMonth(this.lunarYear);
        // 是否闰年
        ...
        while (lunarMonth < 13 && offset > 0) {
            if (isLeap && leapDec) { // 如果是闰年,并且是闰月
                // 所在农历年闰月的天数
                daysInLunarMonth = LunarUtility.getLunarLeapDays(this.lunarYear);
                leapDec = false;
            } else {
                // 所在农历年指定月的天数
                daysInLunarMonth = LunarUtility.getLunarMonthDays(this.lunarYear, lunarMonth);
            }
    ...
    private void getCyclicalData() {
        ...
        int term2 = LunarUtility.getSolarTermDay(solarYear, 2); // 立春日期
        // 依节气调整二月分的年柱, 以立春为界
        ...
        int firstNode = LunarUtility.getSolarTermDay(solarYear, solarMonth * 2); // 传回当月「节」为几日开始
        // 依节气月柱, 以「节」为界
        ...
        cyclicalDay = (int) (LunarUtility.UTC(solarYear, solarMonth, solarDay, 0, 0, 0) / 86400000 + 25567 + 10) % 60;
    ...
    public String getAnimalString() {
        return LunarUtility.Animals[(this.lunarYear - 4) % 12];
    }
    ...
    public String getTermString() {
        // 二十四节气
        String termString = "";
        if (LunarUtility.getSolarTermDay(solarYear, solarMonth * 2) == solarDay) {
            termString = LunarUtility.solarTerm[solarMonth * 2];
        } else if (LunarUtility.getSolarTermDay(solarYear, solarMonth * 2 + 1) == solarDay) {
            termString = LunarUtility.solarTerm[solarMonth * 2 + 1];
        }
        List<String> solarTerms = Arrays.asList(solarTerm_);
        if (solarTerms.contains(termString)) {
            termString = "";
        }
        return termString;
    }
    ...
    public int getTiananY() {
        return LunarUtility.getTianan(this.cyclicalYear);
    }
    ...
    public int getTiananM() {
        return LunarUtility.getTianan(this.cyclicalMonth);
    }
    ...
    public int getTiananD() {
        return LunarUtility.getTianan(this.cyclicalDay);
    }
     ...
     */
    public int getDeqiY() {
        return LunarUtility.getDeqi(this.cyclicalYear);
    }
     ...
     */
    public int getDeqiM() {
        return LunarUtility.getDeqi(this.cyclicalMonth);
    }
     ...
     */
    public int getDeqiD() {
        return LunarUtility.getDeqi(this.cyclicalDay);
    }
    ...
    public String getCyclicaYear() {
        return LunarUtility.getCyclicalString(this.cyclicalYear);
    }
    ...
    public String getCyclicaMonth() {
        return LunarUtility.getCyclicalString(this.cyclicalMonth);
    }
    ...
    public String getCyclicaDay() {
        return LunarUtility.getCyclicalString(this.cyclicalDay);
    }
    ...
    public String getLunarDayString() {
        return LunarUtility.getLunarDayString(this.lunarDay);
    }
    ...
    public String getLunarMonthString() {
        return (this.isLeap() ? "闰" : "") + LunarUtility.getLunarMonthString(this.lunarMonth);
    }
    ...
    public String getLunarYearString() {
        return LunarUtility.getLunarYearString(this.lunarYear);
    }
     ...
     *
     */
    public boolean isBirthday(int bir) {
        Calendar clr = Calendar.getInstance();
        Calendar birClr = Calendar.getInstance();
        birClr.setTimeInMillis(bir * 1000L);
        return clr.get(Calendar.YEAR) == birClr.get(Calendar.YEAR) && clr.get(Calendar.MONTH) == birClr.get(Calendar.MONTH) && clr.get(Calendar.DAY_OF_MONTH) == birClr.get(Calendar.DAY_OF_MONTH);
    }
    ...
    private static String getCyclicalString(int cyclicalNumber) {
        return LunarUtility.Tianan[LunarUtility.getTianan(cyclicalNumber)] + LunarUtility.Deqi[LunarUtility.getDeqi(cyclicalNumber)];
    }
    ...
    private static String getLunarYearString(int lunarYear) {
        return LunarUtility.getCyclicalString(lunarYear - 1900 + 36);
    }
    ...
    private static String getLunarMonthString(int lunarMonth) {
        ...
        if (lunarMonth == 1) {
            lunarMonthString = LunarUtility.lunarString2[4];
        } else {
            if (lunarMonth > 9)
                lunarMonthString += LunarUtility.lunarString2[1];
            if (lunarMonth % 10 > 0)
                lunarMonthString += LunarUtility.lunarString1[lunarMonth % 10];
        }
    ...
    private static String getLunarDayString(int lunarDay) {
        ...
        int i1 = lunarDay / 10;
        int i2 = lunarDay % 10;
        String c1 = LunarUtility.lunarString2[i1];
        String c2 = LunarUtility.lunarString1[i2];
        if (lunarDay < 11)
            c1 = LunarUtility.lunarString2[0];
        if (i2 == 0)
            c2 = LunarUtility.lunarString2[1];
        return c1 + c2;
    }
