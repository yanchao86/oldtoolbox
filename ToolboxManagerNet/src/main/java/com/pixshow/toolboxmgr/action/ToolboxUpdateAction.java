...
public class ToolboxUpdateAction extends BaseAction {
    ...
    public String toolInsert() {
        ...
        if (icon != null) {
            String iconName = UUID.randomUUID() + iconFileName.substring(iconFileName.lastIndexOf("."));
            String uploadPath = Config.getInstance().getString("toolbox.upload.base.folder") + Config.getInstance().getString("toolbox.img.folder") + iconName;
            icon.renameTo(new File(uploadPath));
            bean.setIcon(iconName);
        }
        bean.setName(name);
        bean.setSortIndex(0);
